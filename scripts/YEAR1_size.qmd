...
#plot with speed as categorical data 
ggplot(data=heron, aes(y = Size_log, x = median_speed_cat, color = Spp)) +    
  geom_point()  
```
...
```{r}
marginal_means_log<- emmeans(heron_lmer4Spp, ~median_speed) 
marginal_means_log_df <- as.data.frame(marginal_means)  
head(marginal_means_df)
```
...
# Plot raw data with marginal means and confidence intervals for categorical variable 
ggplot() +   
  geom_point(data = heron, aes(x = median_speed_cat, y = Size, color = Spp), alpha = 0.3) +  # Raw data points with transparency   
  geom_line(data = marginal_means_cat_df, aes(x = median_speed_cat, y = emmean, group = 1), color = "blue", size = 1) +   
  geom_errorbar(data = marginal_means_cat_df, aes(x = median_speed_cat, ymin = lower.CL, ymax = upper.CL), width = 0.2, color = "blue") +   
  theme_minimal() +   
  labs(title = "Raw Data and Marginal Means: Size vs. Median Speed (Categorical)",        
...
```{r}
moore_size_sedturf <-  bf(Size_log ~ sedturf_t5 + (1|Site), family = gaussian())
...
moore.resids5 <- make_brms_dharma_res(moore_brm5_size, integerResponse = FALSE) testUniformity(moore.resids5) 
...
davies_brm5_size_Spp <- brm(davies_size_concrete2,            
                       data=davies,              
                       prior = priors,               
                       sample_prior = 'yes',              
                       iter = 5000,            
...
davies_brm6_size_Spp <- brm(davies_size_turf5,                
                       data=davies,           
                       prior = priors,             
                       sample_prior = 'yes',             
                       iter = 5000,         
...
load(file = "scripts/models/davies/size/davies_brm7_size.RData")
...
```{r}
heron_size_wave <-  bf(Size_log ~ WaveEnergyLevel + (1|Site) + Spp, family = gaussian())
...
```{r}
heron_size_Ub <-  bf(Size_log ~ Ub_avrg + (1|Site) + Spp, family = gaussian())  
...
```{r}
heron_size_speed <-  bf(Size_log ~ median_speed + Spp + (1|Site), family = gaussian())  
...
```{r}
heron_size_turf <-  bf(Size_log ~ sedturf_t2 + Spp + (1|Site), family = gaussian())  
...
```{r}
heron_size_concrete <-  bf(Size_log ~ sedconcrete_t2 + Spp + (1|Site), family = gaussian())  

priors <- prior(normal(0, 5), class = "Intercept") +  # for intercept   
  prior(normal(0,5), class = "b") +                   # for fixed effects - sedconcrete
  prior(cauchy(0,2), class = 'sd')                    # for random effects - Site  
...
```{r}
heron_size_habitat <-  bf(Size_log ~ PC1 + PC2 + Spp + (1|Site), family = gaussian())  

priors <- prior(normal(0, 5), class = "Intercept") +  # for intercept   
  prior(normal(0,5), class = "b") +                   # for fixed effects - PC1 and PC2
  prior(cauchy(0,2), class = 'sd')                    # for random effects - Site  
...
```{r}
load(file = "scripts/models/heron/size/heron_brm1_size.RData") 
load(file = "scripts/models/heron/size/heron_brm2_size.RData") 
load(file = "scripts/models/heron/size/heron_brm3_size.RData") 
load(file = "scripts/models/heron/size/heron_brm4_size.RData") 
load(file = "scripts/models/heron/size/heron_brm5_size.RData") 
load(file = "scripts/models/heron/size/heron_brm6_size.RData")
...
looic(heron_brm1_size_Spp)      #  LOOIC: 367.82 [14.18]
looic(heron_brm2_size_Spp)      #  LOOIC: 368.35 [13.74]
looic(heron_brm3_size_Spp)      #  LOOIC: 367.00 [14.19]
looic(heron_brm4_size_Spp)      #  LOOIC: 365.94 [13.49]
looic(heron_brm5_size_Spp)      #  LOOIC: 368.61 [13.58]
looic(heron_brm6_size_Spp)      #  LOOIC: 365.07 [13.17] 
...
size_allreef <- rbind(heron, moore, davies)
```
...
plot_size_site <- ggplot(size_allreef, aes(x= Site, y = Size, color = Spp)) +
  ...
  labs(title = "Coral Size Distribution Across Sites and Reefs",
       x = "Site",
       y = "Coral Size (mm3)",
       color = "Species") +
  theme_bw() +
...
plot_size_site
ggsave("Fig3b - Coral Size by site.png", plot=plot_size_site, width = 14, height = 6, dpi = 300)
```
...
ggplot() +  
  geom_point(data = moore, aes(x = Ub_avrg, y = Size_log), alpha = 0.5, color = "green") +    
  geom_point(data = predicted_data_moore_brm2, aes(x = Ub_avrg, y = Fitted, color = Spp), alpha = 0.5) +    
  geom_line(data = predicted_data_moore_brm2, aes(x = Ub_avrg, y = Fitted, group = Spp, color = Spp)) +   
  geom_ribbon(data = predicted_data_moore_brm2, aes(x = Ub_avrg, ymin = Lower, ymax = Upper, fill = Spp), alpha = 0.2) +   
  labs(title = "Moore Reef",    
       x = "Bottom stress (m/s)",    
       y = "Predicted growth (mm3)", 
       color = "Species", 
       fill = "Species") +  
  scale_color_manual(values = c("A_millepora" = "darkgreen")) +
  scale_fill_manual(values = c("A_millepora" = "green")) +
  theme_minimal() + 
...
plot_moore_Ub <- ggplot() +
  ...
  geom_point(data = moore, aes(x = Ub_avrg, y = Size_log), color = "green", alpha = 0.5) +
  # Plot the predicted values with error bars, colored in dark green
  geom_point(data = predicted_data_moore_brm2, aes(x = Ub_avrg, y = Fitted), color = "darkgreen") +
  geom_errorbar(data = predicted_data_moore_brm2, aes(x = Ub_avrg, ymin = Lower, ymax = Upper), color = "darkgreen", width = 0.01, alpha = 0.5) +
  labs(
    title = "Moore Reef",
    x = "Bottom stress (m/s)",
    y = "Predicted growth (mm3)",
...
# Create the plot with points, error bars, and raw data 
ggplot() +  
  geom_point(data = moore, aes(x = WaveEnergyLevel, y = Size_log), alpha = 0.5, color = "green") +    
  geom_point(data = predicted_data_moore_brm1, aes(x = WaveEnergyLevel, y = Fitted, color = Spp), alpha = 0.5) +    
  geom_line(data = predicted_data_moore_brm1, aes(x = WaveEnergyLevel, y = Fitted, group = Spp, color = Spp)) +   
  geom_ribbon(data = predicted_data_moore_brm1, aes(x = WaveEnergyLevel, ymin = Lower, ymax = Upper, fill = Spp), alpha = 0.2) +   
  labs(title = "Moore Reef",    
...
plot_moore_WaveEnergyLevel <- ggplot() +
  # Plot the raw data points, colored in green
  geom_point(data = moore, aes(x = WaveEnergyLevel, y = Size_log), color = "green", alpha = 0.5) +
  # Plot the predicted values with error bars, colored in dark green
  geom_point(data = predicted_data_moore_brm1, aes(x = WaveEnergyLevel, y = Fitted), color = "darkgreen") +
  geom_errorbar(data = predicted_data_moore_brm1, aes(x = WaveEnergyLevel, ymin = Lower, ymax = Upper), color = "darkgreen", width = 0.1, alpha = 0.5) +
  labs(
    title = "Moore Reef",
    x = "Nominal Wave Energy Level",
    y = "Predicted growth (mm3)",
...
# Create the plot with points, error bars, and raw data 
ggplot() +  
  geom_point(data = moore, aes(x = median_speed, y = Size_log), alpha = 0.5, color = "green") +    
  geom_point(data = predicted_data_moore_brm3, aes(x = median_speed, y = Fitted, color = Spp), alpha = 0.5) +    
  geom_line(data = predicted_data_moore_brm3, aes(x = median_speed, y = Fitted, group = Spp, color = Spp)) +   
  geom_ribbon(data = predicted_data_moore_brm3, aes(x = median_speed, ymin = Lower, ymax = Upper, fill = Spp), alpha = 0.2) +   
  labs(title = "Moore Reef",    
       x = "Median Flow velocity (m/s)",    
       y = "Predicted growth (mm3)", 
       color = "Species", 
...
plot_moore_speed <- ggplot() +
  # Plot the raw data points, colored in green
  geom_point(data = moore, aes(x = median_speed, y = Size_log), color = "green", alpha = 0.5) +
  # Plot the predicted values with error bars, colored in dark green
  geom_point(data = predicted_data_moore_brm3, aes(x = median_speed, y = Fitted), color = "darkgreen") +
  geom_errorbar(data = predicted_data_moore_brm3, aes(x = median_speed, ymin = Lower, ymax = Upper), color = "darkgreen", width = 0.002, alpha = 0.5) +
  labs(
    title = "Moore Reef",
    x = "Median Flow velocity (m s-1)",
    y = "Predicted growth (mm3)",
    color = "Species"
  ) +
  theme_minimal() +
...
# Create the plot with points, error bars, and raw data 
ggplot() +  
  geom_point(data = moore, aes(x = sedturf_t5, y = Size_log), alpha = 0.5, color = "green") +    
  geom_point(data = predicted_data_moore_brm4, aes(x = sedturf_t5, y = Fitted, color = Spp), alpha = 0.5) +    
  geom_line(data = predicted_data_moore_brm4, aes(x = sedturf_t5, y = Fitted, group = Spp, color = Spp)) +   
  geom_ribbon(data = predicted_data_moore_brm4, aes(x = sedturf_t5, ymin = Lower, ymax = Upper, fill = Spp), alpha = 0.2) +   
  labs(title = "Moore Reef",    
...
plot_moore_turf <- ggplot() +
  # Plot the raw data points, colored in green
  geom_point(data = moore, aes(x = sedturf_t5, y = Size_log), color = "green", alpha = 0.5) +
  # Plot the predicted values with error bars, colored in dark green
  geom_point(data = predicted_data_moore_brm4, aes(x = sedturf_t5, y = Fitted), color = "darkgreen") +
  geom_errorbar(data = predicted_data_moore_brm4, aes(x = sedturf_t5, ymin = Lower, ymax = Upper), color = "darkgreen", width = 0.05, alpha = 0.5) +
  labs(
    title = "Moore Reef",
    x = "Sedimentation on turf (mg cm-2)",    
    y = "Predicted growth (mm3)",
...
# Create the plot with points, error bars, and raw data 
ggplot() +  
  geom_point(data = moore, aes(x = sedconcrete_t5, y = Size_log), alpha = 0.5, color = "green") +    
  geom_point(data = predicted_data_moore_brm5, aes(x = sedconcrete_t5, y = Fitted, color = Spp), alpha = 0.5) +    
  geom_line(data = predicted_data_moore_brm5, aes(x = sedconcrete_t5, y = Fitted, group = Spp, color = Spp)) +   
  geom_ribbon(data = predicted_data_moore_brm5, aes(x = sedconcrete_t5, ymin = Lower, ymax = Upper, fill = Spp), alpha = 0.2) +   
  labs(title = "Moore Reef",    
...
plot_moore_concrete <- ggplot() +
  # Plot the raw data points, colored in green
  geom_point(data = moore, aes(x = sedconcrete_t5, y = Size_log), color = "green", alpha = 0.5) +
  # Plot the predicted values with error bars, colored in dark green
  geom_point(data = predicted_data_moore_brm5, aes(x = sedconcrete_t5, y = Fitted), color = "darkgreen") +
  geom_errorbar(data = predicted_data_moore_brm5, aes(x = sedconcrete_t5, ymin = Lower, ymax = Upper), color = "darkgreen", width = 0.02, alpha = 0.5) +
  labs(
    title = "Moore Reef",
    x = "Sedimentation on concrete (mg cm-2)",
    y = "Predicted growth (mm3)",
...
# Create the plot with points, error bars, and raw data 
ggplot() +  
  geom_point(data = davies, aes(x = Ub_avrg, y = Size_log), alpha = 0.5, color = "orange") +    
  geom_point(data = predicted_data_davies_brm2, aes(x = Ub_avrg, y = Fitted, color = Spp), alpha = 0.5) +    
  geom_line(data = predicted_data_davies_brm2, aes(x = Ub_avrg, y = Fitted, group = Spp, color = Spp)) +   
  geom_ribbon(data = predicted_data_davies_brm2, aes(x = Ub_avrg, ymin = Lower, ymax = Upper, fill = Spp), alpha = 0.2) +   
  ...
  theme_minimal() + 
  theme(panel.border = element_rect(color = "black", fill = NA, linewidth = 0.5),           axis.ticks = element_line(color = "black", linewidth = 0.5), 
        axis.ticks.length = unit(0.25, "cm") # Adjust tick length as needed  
  )
```
...
plot_davies_Ub <- ggplot() +
  ...
  geom_point(data = davies, aes(x = Ub_avrg, y = Size_log), color = "orange", alpha = 0.5) +
  # Plot the predicted values with error bars, colored in dark orange
  geom_point(data = predicted_data_davies_brm2, aes(x = Ub_avrg, y = Fitted), color = "darkorange") +
  geom_errorbar(data = predicted_data_davies_brm2, aes(x = Ub_avrg, ymin = Lower, ymax = Upper), color = "darkorange", width = 0.02, alpha = 0.5) +
  labs(
    title = "Davies Reef",
    x = "Bottom stress (m/s)",
    y = "Predicted growth (mm3)",
...
# Create the plot with points, error bars, and raw data 
ggplot() +  
  geom_point(data = davies, aes(x = WaveEnergyLevel, y = Size_log), alpha = 0.5, color = "orange") +    
  geom_point(data = predicted_data_davies_brm1, aes(x = WaveEnergyLevel, y = Fitted, color = Spp), alpha = 0.5) +    
  geom_line(data = predicted_data_davies_brm1, aes(x = WaveEnergyLevel, y = Fitted, group = Spp, color = Spp)) +   
  geom_ribbon(data = predicted_data_davies_brm1, aes(x = WaveEnergyLevel, ymin = Lower, ymax = Upper, fill = Spp), alpha = 0.2) +   
  labs(title = "Davies Reef",    
       x = "Nominal Wave Energy Level",    
       y = "Predicted growth (mm3)", 
       color = "Species", 
...
plot_davies_WaveEnergyLevel <- ggplot() +
  # Plot the raw data points, colored in orange
  geom_point(data = davies, aes(x = WaveEnergyLevel, y = Size_log), color = "orange", alpha = 0.5) +
  # Plot the predicted values with error bars, colored in dark orange
  geom_point(data = predicted_data_davies_brm1, aes(x = WaveEnergyLevel, y = Fitted), color = "darkorange") +
  geom_errorbar(data = predicted_data_davies_brm1, aes(x = WaveEnergyLevel, ymin = Lower, ymax = Upper), color = "darkorange", width = 0.1, alpha = 0.5) +
  labs(
    title = "Davies Reef",
    x = "Nominal Wave Energy Level",    
    y = "Predicted growth (mm3)",
...
# Create the plot with points, error bars, and raw data 
ggplot() +  
  geom_point(data = davies, aes(x = median_speed, y = Size_log), alpha = 0.5, color = "orange") +    
  geom_point(data = predicted_data_davies_brm3, aes(x = median_speed, y = Fitted, color = Spp), alpha = 0.5) +    
  geom_line(data = predicted_data_davies_brm3, aes(x = median_speed, y = Fitted, group = Spp, color = Spp)) +   
  geom_ribbon(data = predicted_data_davies_brm3, aes(x = median_speed, ymin = Lower, ymax = Upper, fill = Spp), alpha = 0.2) +   
  labs(title = "Davies Reef",    
       x = "Median Flow velocity (m/s)",    
       y = "Predicted growth (mm3)", 
       color = "Species", 
...
plot_davies_speed <- ggplot() +
  # Plot the raw data points, colored in orange
  geom_point(data = davies, aes(x = median_speed, y = Size_log), color = "orange", alpha = 0.5) +
  # Plot the predicted values with error bars, colored in dark orange
  geom_point(data = predicted_data_davies_brm3, aes(x = median_speed, y = Fitted), color = "darkorange") +
  geom_errorbar(data = predicted_data_davies_brm3, aes(x = median_speed, ymin = Lower, ymax = Upper), color = "darkorange", width = 0.005, alpha = 0.5) +
  labs(
    title = "Davies Reef",
    x = "Median Flow velocity (m/s)",    
    y = "Predicted growth (mm3)",
...
# Create the plot with points, error bars, and raw data 
ggplot() +  
  geom_point(data = davies, aes(x = sedturf_t2, y = Size_log), alpha = 0.5, color = "orange") +    
  geom_point(data = predicted_data_davies_brm4, aes(x = sedturf_t2, y = Fitted, color = Spp), alpha = 0.5) +    
  geom_line(data = predicted_data_davies_brm4, aes(x = sedturf_t2, y = Fitted, group = Spp, color = Spp)) +   
  geom_ribbon(data = predicted_data_davies_brm4, aes(x = sedturf_t2, ymin = Lower, ymax = Upper, fill = Spp), alpha = 0.2) +   
  labs(title = "Davies Reef",    
       x = "Sedimentation on turf (mg cm-2)",    
       y = "Predicted growth (mm3)", 
       color = "Species", 
...
plot_davies_turf <- ggplot() +
  # Plot the raw data points, colored in orange
  geom_point(data = davies, aes(x = sedturf_t2, y = Size_log), color = "orange", alpha = 0.5) +
  # Plot the predicted values with error bars, colored in dark orange
  geom_point(data = predicted_data_davies_brm4, aes(x = sedturf_t2, y = Fitted), color = "darkorange") +
  geom_errorbar(data = predicted_data_davies_brm4, aes(x = sedturf_t2, ymin = Lower, ymax = Upper), color = "darkorange", width = 0.05, alpha = 0.5) +
  labs(
    title = "Davies Reef",
    x = "Sedimentation on turf (mg cm-2)",    
    y = "Predicted growth (mm3)",
...
predicted_data_davies_brm5 <- data.frame(  
  sedconcrete_t2 = davies$sedconcrete_t2,  
  Spp = davies$Spp,  
...
# Create the plot with points, error bars, and raw data 
ggplot() +  
  geom_point(data = davies, aes(x = sedconcrete_t2, y = Size_log), alpha = 0.5, color = "orange") +    
  geom_point(data = predicted_data_davies_brm5, aes(x = sedconcrete_t2, y = Fitted, color = Spp), alpha = 0.5) +    
