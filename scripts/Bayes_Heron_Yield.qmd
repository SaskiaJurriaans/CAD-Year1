...
Best model is glm with median_speed
...
heron_form_sed <- bf(SurvDev ~ sedturf_t2 + median_speed + (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
heron_form_Wave <- bf(SurvDev ~ WaveEnergyLevel + median_speed + (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
...
priors_sed <- prior(normal(0, 1.7), class = "Intercept") +
...
# To load the model back in workspace
# MAY NEED TO RUN THIS IN CONSOLE TO WORK
...
# To load the model back in workspace
#MAY NEED TO RUN THIS IN CONSOLE TO WORK
#load(file = "scripts/models/scripts/models/heron_brm_Ub.RData")
...
```{r}
heron_brm_sed <- brm(heron_form_sed,
                 data=heron_t2,
                 prior = priors_sed,
                 sample_prior = 'yes',
                 iter = 5000,
...
# Save the model 
#save(heron_brm_sed, file = "scripts/models/heron_brm_sed.RData")

# To load the model back in workspace
#MAY NEED TO RUN THIS IN CONSOLE TO WROK
#load(file = "scripts/models/heron_brm_sed.RData")
...
```{r}
heron_brm$fit |> stan_trace()
heron_brm$fit |> stan_ac()
heron_brm$fit |> stan_rhat()
heron_brm$fit |> stan_ess()
```
