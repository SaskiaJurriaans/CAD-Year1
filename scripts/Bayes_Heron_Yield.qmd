...
heron_Spp_t2 <- heron_t2 |> select(-c(sedturf_t5, sedconcrete_t5))
```
...
OUTPUT: all models behave very similar and are better than a model with species in it
...
heron_form_Ub <-    bf(SurvDev ~ Ub_avrg +         (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
heron_form_turf <-   bf(SurvDev ~ sedturf_t2 +     (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
heron_form_concrete <-bf(SurvDev ~ sedconcrete_t2 +(1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
heron_form_Wave <-  bf(SurvDev ~ WaveEnergyLevel + (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
heron_form_PC1 <-   bf(SurvDev ~ PC1 +             (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
```
...
```{r}
heron_form <- bf(SurvDev ~ (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
heron_form_speed_poly <- bf(SurvDev ~ poly(median_speed,3) +    (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
heron_form_Ub_poly <-    bf(SurvDev ~ poly(Ub_avrg,3) +         (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
heron_form_turf_poly <-   bf(SurvDev ~ poly(sedturf_t2,3) +      (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
heron_form_Wave_poly <-  bf(SurvDev ~ poly(WaveEnergyLevel,3) + (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
heron_form_PC1_poly <-   bf(SurvDev ~ poly(PC1,3) +             (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
```
...
```{r}
2.5 * sd(heron_t2$SurvDev, na.rm = TRUE)/sd(heron_t2$Ub_avrg, na.rm = TRUE)
...
```{r}
2.5 * sd(heron_t2$SurvDev, na.rm = TRUE)/sd(heron_t2$WaveEnergyLevel, na.rm = TRUE)
...
```{r}
2.5 * sd(heron_t2$SurvDev, na.rm = TRUE)/sd(heron_t2$PC1, na.rm = TRUE)
...
load(file = "scripts/models/heron_brm_PC1.RData")
```
...
heron_t2 <- heron_t2[complete.cases(heron_t2$median_speed),  ]
