...
What was the average survival at Moore and DAvies Reef after 1 yr (census 4) and what was the average survival at Heron Reef after 1 yr (census 2?

```{r}
summary_survival_device %>%
  filter(
    (Reef %in% c("Moore", "Davies") & Census == "t4") |
    (Reef == "Heron" & Census == "t2")
  ) %>%
  group_by(Reef, Spp) %>%
  summarise(
    avg_percent_survival = mean(Perc_SurvDev, na.rm = TRUE),
    n = n()
  )
