...
```{r}
davies_t5 <- merge(davies_env, davies_surv, by = c("Reef", "Site", "ReefDev"), all = TRUE)
...
summary(davies_glmer1)
summary(davies_glmer2)
summary(davies_glmer3)
summary(davies_glmer4)
summary(davies_glmer5)

AIC(davies_glmer1, davies_glmer2, davies_glmer3, davies_glmer4, davies_glmer5)
```
...
```{r}
davies_device <- bf(SurvDev ~ (1|Site),  family = bernoulli(link='logit'))

davies_form <- bf(SurvDev ~ (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))

davies_form_speed <- bf(SurvDev ~ median_speed +    (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))

davies_form_Ub <-    bf(SurvDev ~ Ub_avrg +         (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
davies_form_turf <-   bf(SurvDev ~ sedturf_t5 +     (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
davies_form_concrete <-bf(SurvDev ~ sedconcrete_t5 +(1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
davies_form_Wave <-  bf(SurvDev ~ WaveEnergyLevel + (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
davies_form_PC1 <-   bf(SurvDev ~ PC1 +             (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
```
...
```{r}
davies_form <- bf(SurvDev ~ (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
davies_form_speed_poly <- bf(SurvDev ~ poly(median_speed,3) +    (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
davies_form_Ub_poly <-    bf(SurvDev ~ poly(Ub_avrg,3) +         (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
davies_form_turf_poly <-   bf(SurvDev ~ poly(sedturf_t5,3) +      (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
davies_form_Wave_poly <-  bf(SurvDev ~ poly(WaveEnergyLevel,3) + (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
davies_form_PC1_poly <-   bf(SurvDev ~ poly(PC1,3) +             (1|Site) + (1|ReefDev), family = bernoulli(link='logit'))
```
...
#save(davies_brm, file = "scripts/models/davies_brm.RData")
```
...
# Save the model
#save(davies_brm_speed, file = "scripts/models/davies_brm_speed.RData")
```
...
# Save the model
#save(davies_brm_Ub, file = "scripts/models/davies_brm_Ub.RData")
```
...
# Save the model
#save(davies_brm_wave, file = "scripts/models/davies_brm_wave.RData")
```
...
# Save the model 
#save(davies_brm_turf, file = "scripts/models/davies_brm_turf.RData")
```
...
# Save the model 
#save(davies_brm_concrete, file = "scripts/models/davies_brm_concrete.RData")
```

### brm- PC1

```{r}
davies_brm_PC1 <- brm(davies_form_PC1,
                 data=davies_t5,
                 prior = priors_PC1,
                 sample_prior = 'yes',
                 iter = 5000,
...
# Save the model 
#save(davies_brm_PC1, file = "scripts/models/davies_brm_PC1.RData")
```

### Load models

```{r}
load(file = "scripts/models/davies_brm.RData")
load(file = "scripts/models/davies_brm_speed.RData")
load(file = "scripts/models/davies_brm_Ub.RData")
load(file = "scripts/models/davies_brm_wave.RData")
load(file = "scripts/models/davies_brm_turf.RData")
load(file = "scripts/models/davies_brm_concrete.RData")
load(file = "scripts/models/davies_brm_PC1.RData")
```

### loo

```{r}l}
l_brm <- davies_brm |> loo()
l_speed <- davies_brm_speed|> loo()
l_Ub <- davies_brm_Ub |> loo()
l_wave <- davies_brm_wave |> loo()
l_turf <- davies_brm_turf|> loo()
l_concrete <- davies_brm_concrete|> loo()
l_PC1 <- davies_brm_PC1|> loo()

looic(davies_brm)           #  LOOIC: 257.06 [15.59]
looic(davies_brm_speed)     #  LOOIC: 256.87 [15.57]
looic(davies_brm_wave)      #  LOOIC: 253.67 [15.30]
looic(davies_brm_concrete)  #  LOOIC: 254.73 [15.45]
looic(davies_brm_turf)      #  LOOIC: 255.66 [15.56]
looic(davies_brm_PC1)       #  LOOIC: 255.66 [15.56]
...
```{r}
pars <- davies_brm |> get_variables()  #capture the list of variables
pars <- str_extract(pars, "^b_.*|^sd.*") |> na.omit()
pars

davies_brm $fit |> stan_trace(pars = pars)
davies_brm $fit |> stan_ac(pars = pars)
davies_brm$fit |> stan_rhat()
davies_brm$fit |> stan_ess()  # the closer to 1, the more chains have converged
davies_brm |> mcmc_plot(type='violin')  # assess convergence and mixing of MCMC samples and skewness of the distribution
```
...
-   **stan_ess:** effective sample size not very efficient and split in two groups of frequencies?

#### **Posterior probability checks**

```{r}
davies_brm |> pp_check(type = 'dens_overlay', ndraws = 100)
davies_brm |> pp_check (type = "error_scatter_avg")
davies_brm  |> pp_check(group = 'Site', type = 'violin_grouped')
```
...
```{r}
library(DHARMa)
davies.resids <- make_brms_dharma_res(davies_brm, integerResponse = FALSE)
testUniformity(davies.resids)
```

```{r}
wrap_elements(~testUniformity(davies.resids))
wrap_elements(~plotResiduals(davies.resids, form = factor(rep(1,nrow(davies_t5))))) 
...
```{r}
pars <- davies_brm_speed |> get_variables()  #capture the list of variables
pars <- str_extract(pars, "^b_.*|^sd.*") |> na.omit()
pars

davies_brm_speed$fit |> stan_trace(pars = pars)
davies_brm_speed$fit |> stan_ac(pars = pars)
davies_brm_speed$fit |> stan_rhat()
davies_brm_speed$fit |> stan_ess()
```

-   **stan_ac**: All below 0.25, all good.
-   **rhat_hist**: Rhat is a **scale reduction factor** measure of convergence between the chains. The closer the values are to 1, the more the chains have converged. Values greater than 1.05 indicate a lack of convergence. There will be an Rhat value for each parameter estimated.
-   **stan_ess:** effective sample size not good, also split into two distinct groups

#### **Posterior probability checks**

```{r}
davies_brm_speed |> pp_check(type = 'dens_overlay', ndraws = 100)
davies_brm_speed |> pp_check (type = "error_scatter_avg")
davies_brm_speed |> mcmc_plot(type='violin')
```
...
```{r}
library(DHARMa)
davies.resids <- make_brms_dharma_res(davies_brm_speed, integerResponse = TRUE)
testUniformity(davies.resids)
```

```{r}
#remove NA values of predictor variable
davies_t5 <- davies_t5[complete.cases(davies_t5$median_speed),  ]
...
```{r}
pars <- davies_brm_Ub |> get_variables()  #capture the list of variables
pars <- str_extract(pars, "^b_.*|^sd.*") |> na.omit()
pars

davies_brm_Ub$fit |> stan_trace(pars = pars)
davies_brm_Ub$fit |> stan_ac(pars = pars)
davies_brm_Ub$fit |> stan_rhat()
davies_brm_Ub$fit |> stan_ess()
```
...
davies_brm_Ub |> pp_check (type = "error_scatter_avg")
davies_brm_Ub |> mcmc_plot(type='violin')
```
...
```{r}
library(DHARMa)
davies.resids <- make_brms_dharma_res(davies_brm_Ub, integerResponse = TRUE)
testUniformity(davies.resids)
```
...
```{r}
pars <- davies_brm_wave |> get_variables()  #capture the list of variables
pars <- str_extract(pars, "^b_.*|^sd.*") |> na.omit()
pars

davies_brm_wave$fit |> stan_trace(pars = pars)
davies_brm_wave$fit |> stan_ac(pars = pars)
davies_brm_wave$fit |> stan_rhat()
davies_brm_wave$fit |> stan_ess()
```
...
davies_brm_wave |> pp_check (type = "error_scatter_avg")
davies_brm_wave |> mcmc_plot(type='violin')
```
...
```{r}
library(DHARMa)
davies.resids <- make_brms_dharma_res(davies_brm_wave, integerResponse = FALSE)
testUniformity(davies.resids)
```
...
```{r}
pars <- davies_brm_turf |> get_variables()  #capture the list of variables
pars <- str_extract(pars, "^b_.*|^sd.*") |> na.omit()

davies_brm_turf$fit |> stan_trace(pars = pars)
davies_brm_turf$fit |> stan_ac(pars = pars)
davies_brm_turf$fit |> stan_rhat()
davies_brm_turf$fit |> stan_ess()
```
...
davies_brm_turf |> pp_check (type = "error_scatter_avg")
davies_brm_turf |> mcmc_plot(type='violin')
```
...
```{r}
library(DHARMa)
davies.resids <- make_brms_dharma_res(davies_brm_turf, integerResponse = FALSE)
testUniformity(davies.resids)
```
...
```{r}
pars <- davies_brm_PC1 |> get_variables()  #capture the list of variables
pars <- str_extract(pars, "^b_.*|^sd.*") |> na.omit()


davies_brm_PC1$fit |> stan_trace(pars = pars)
davies_brm_PC1$fit |> stan_ac(pars = pars)
davies_brm_PC1$fit |> stan_rhat()
davies_brm_PC1$fit |> stan_ess()
```

#### **Posterior probability checks**

```{r}
davies_brm_PC1 |> pp_check(type = 'dens_overlay', ndraws = 100)
davies_brm_PC1 |> pp_check (type = "error_scatter_avg")
davies_brm_PC1 |> mcmc_plot(type='violin')
```
...
```{r}
library(DHARMa)
davies.resids <- make_brms_dharma_res(davies_brm_PC1, integerResponse = FALSE)
testUniformity(davies.resids)
```
...
davies_brm_speed |> conditional_effects() |> plot(points=TRUE)
davies_brm_Ub |> conditional_effects() |> plot(points=TRUE)
davies_brm_turf |> conditional_effects() |> plot(points=TRUE)
davies_brm_wave |> conditional_effects() |> plot(points=TRUE)
davies_brm_PC1 |> conditional_effects() |> plot(points=TRUE)
```
...
ggpredict(davies_brm_speed, terms = ~median_speed) %>%
   plot(add.data = TRUE)
 

Fig1a <- ggemmeans(davies_brm_speed, terms = ~median_speed) %>%
  plot(add.data = TRUE) 

conditional_effects(davies_brm_speed) |> 
  plot(points = TRUE)

Fig1b <- davies_brm_speed %>% emmeans(~median_speed , type = 'link') %>%
    as.data.frame() %>%
    ggplot(aes(y = emmean, x = median_speed)) +
   # geom_hline(yintercept = c(5,-5), linetype = 'dashed') +
    geom_pointrange(aes(ymin = lower.HPD, ymax = upper.HPD)) +
    geom_point(data = davies_t5, aes(y = SurvDev),
...
```{r}
davies_brm_speed |> summary()
```

```{r}
davies_brm_speed$fit |> tidyMCMC(estimate.method = "median", 
                            conf.int = TRUE, 
...
```{r}
davies_speed_tidy <- davies_brm_speed |> as_draws_df()  |>
...
#RUN IN CONSOLE
#write.xlsx(davies.speed.brm.tidy, file = "data/davies survival-speed brm tidy_odds ratio.xlsx") #Saving this data for interpretation in a manuscript
...
```{r}
davies_brm_Ub |> summary()
```

### Wave

```{r}
davies_brm_wave |> summary()
```

### Turf

```{r}
davies_brm_turf |> summary()
```

### Concrete

```{r}
davies_brm_concrete |> summary()
```
...
models <- list(
  davies_brm = davies_brm,
  davies_brm_speed = davies_brm_speed,
  davies_brm_wave = davies_brm_wave,
  davies_brm_Ub = davies_brm_Ub,
  davies_brm_turf = davies_brm_turf,
  davies_brm_concrete = davies_brm_concrete,
  davies_brm_PC1 = davies_brm_PC1
)
...
davies_brm_wave |> 
  ...
  mutate(LD50 = -1*b_Intercept / b_WaveEnergyLevel) |>
  median_hdci(LD50)
```
